classdef appClass < handle
    ...
    properties (Access = public)
        ...
        tempPeakTrackingPlotCropValues; % shon -- 2 April 2016 -- total hack to get this working -- bug in plot reagent function
...
%                        self.dataset{self.appParams.activeChannel,scanNum}.peaks{self.appParams.activePeak}.fitPeak();
        ...
        function self = peakTrackingFigCropLB_callback (self, hObject, ~)
            ...
            self.peakTrackingPlotCropValues(self.LB) = str2double(numStr);
            self.updatePlotting();
        end
        
        function self = peakTrackingFigCropUB_callback (self, hObject, ~)
            numStr = get(hObject, 'String');
            self.peakTrackingPlotCropValues(self.UB) = str2double(numStr);
            self.updatePlotting();
        end
...
%             self.appParams.fitPeakTracking = S.app.fitPeakTracking;
        ...
        function self = plotPeakTracking(self)
            % variables to save data to disk each time -- 2 April 2016
            yRefCh = [];
            refChStr = '';
            activeChStr = '';
            reagents= [];
            ...
            self.tempPeakTrackingPlotCropValues = self.peakTrackingPlotCropValues;
...
%                 tempFitPeakTrackingArray{self.appParams.activeChannel}{self.appParams.activePeak}*yScale;
            ...
            if ~self.contextMenu.trackedPeaksPlot.plotExcludedScans
                ...
                leftExclude = sum(self.appParams.tempActiveChannelExcludedScans < self.tempPeakTrackingPlotCropValues(self.LB));
                rightExclude = sum(self.appParams.tempActiveChannelExcludedScans <= self.tempPeakTrackingPlotCropValues(self.UB));
                self.tempPeakTrackingPlotCropValues(self.LB) = self.tempPeakTrackingPlotCropValues(self.LB) - leftExclude;
                self.tempPeakTrackingPlotCropValues(self.UB) = self.tempPeakTrackingPlotCropValues(self.UB) - rightExclude;
            end
...
%                         tempXData, self.scanTemperatures);
                    [self.gui.peakTrackingAndTempFig, h1, h2] = plotyy(self.gui.peakTrackingFig(1),...
                        tempXData(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB)),...
                        [tempFitPeakTrackingArray(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB));...
                        tempRawPeakTrackingArray(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB))], ...
                        tempXData(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB)),...
                        tempScanTemperature(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB)));
                else
%                     [ax, h1, h2] = plotyy(self.gui.peakTrackingFig(1), ...
...
%                     plot(self.gui.peakTrackingFig(1), tempXData, tempFitPeakTrackingArray{self.appParams.activeChannel}{self.appParams.activePeak});
                    ...
                    if self.contextMenu.trackedPeaksPlot.plotReference
                        refChPlot_h = plot(self.gui.peakTrackingFig(1),...
                            tempXData(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB)),...
                            tempReferenceFitPeakTrackingArray(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB)));
                        set(refChPlot_h, 'Color', 'red', 'LineStyle', '--', 'Linewidth', 1);
                        % this hold needs to come after the re-plot to
                        % clear it - shon - 29 March 2016
                        hold(self.gui.peakTrackingFig(1), 'on');
                        % to save temp data to disk each time
                        yRefCh = tempReferenceFitPeakTrackingArray(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB));
                    end
...
%             end
            ...
            % Save what's in the peak tracking window to a file -- quick hack to export data for figures...
            fileName = '_temp_peakTrackingWindowDataExport.mat';
            x = tempXData(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB));
            yActiveCh = tempFitPeakTrackingArray(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB));
            yTemperature = tempScanTemperature(self.tempPeakTrackingPlotCropValues(self.LB):self.tempPeakTrackingPlotCropValues(self.UB));
%                 'yRefCh', yRefCh, 'reagents', reagents,...
%                 'yRefChLabel', refChStr, 'yActiveChLabel', activeChStr, 'xLabelName', xLabelName);
            save([self.path.datasetDir, fileName], 'x', 'yActiveCh', 'yTemperature', 'yRefCh', 'reagents', 'refChStr', 'refChStr', 'activeChStr', 'xLabelName');
        ...
        function reagents = showReagents(self)
            ...
            count = 1;  %need to have a length(condition) = 1 to start with
            for rc = 2:length(tempReagentChangeIndex)
                condition = find(self.appParams.tempActiveChannelExcludedScans(count:end) <= self.reagentChangeIndex(rc));
                if ~isempty(condition)
                    tempReagentChangeIndex(rc:end) = tempReagentChangeIndex(rc:end) - length(condition);
                    count = count + length(condition);
                end
            end
            ...
            for rc = 2:length(tempReagentChangeIndex)
                ...
                if self.tempPeakTrackingPlotCropValues(self.LB) <= tempReagentChangeIndex(rc) &&...
                        tempReagentChangeIndex(rc) <= self.tempPeakTrackingPlotCropValues(self.UB)
                    plot(self.gui.peakTrackingFig(1), self.appParams.xData(rcIndex)*ones(10, 1), linspace(yLimit(1), yLimit(2), 10), 'k--');
                    text((self.appParams.xData(previousRcIndex) + self.appParams.xData(rcIndex))/2, yLimit(1) + (yLimit(2) - yLimit(1)) * textOffsetScaler(mod(rc,2)+1), ...
...
%             return
        ...
        function scanlineCorrelation(self)
            ...
            if self.appParams.activeScan > self.appParams.referenceToPlot
                ...
                if (self.appParams.activeScan - 1) <= 0
                    comparativeScanIndex = self.appParams.activeScan; % condition for at the beginning of the array
                else
                    comparativeScanIndex = self.appParams.activeScan - 1;
                end
                    thisScanWvlSize = length(self.dataset{self.appParams.activeChannel, comparativeScanIndex}.wvl);
                ...
                if thisScanWvlSize == nextScanWvlSize
                    ...
                    [rho,pval] = corr(...
                        self.dataset{self.appParams.activeChannel, self.appParams.activeScan}.pwr,...
                        self.dataset{self.appParams.activeChannel, comparativeScanIndex}.pwr);

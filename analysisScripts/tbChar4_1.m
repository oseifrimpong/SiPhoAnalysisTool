...
channel = 2; %data is in channel 2
    ...
    for pIndex = 1:obj.datasetParams.numOfPeaks(channel)
        ...
        fitPeaks_pwr = [];
        motor_posx_1 = [];
        motor_posx_2 = []; %stage position is saved twice. wonder if the same.
        motor_posy_1 = [];
        motor_posy_2 = []; %stage position is saved twice. wonder if the same. 
        ...
        for scanNumber = obj.firstScanNumber:obj.lastScanNumber
            if ~any(scanNumber == obj.appParams.tempActiveChannelExcludedScans)
                ...
                motor_posx_1 = [motor_posx_1, obj.dataset{channel, scanNumber}.params.opticalStagePosX];
                motor_posy_1 = [motor_posy_1, obj.dataset{channel, scanNumber}.params.opticalStagePosY];
                motor_posx_2 = [motor_posx_2, obj.dataset{channel, scanNumber}.params.opticalStageParams.xmotpos];
                motor_posy_2 = [motor_posy_2, obj.dataset{channel, scanNumber}.params.opticalStageParams.ymotpos];
            end
    ...
    if obj.dataset{channel, obj.firstScanNumber}.params.DisableStageActiveFB
        act_str = 'active feedback disabled';
    else
        act_str = 'active feedback enabled'
    end
    
    %Plot scatter matrix
    figure; 
    scatter((motor_posx_1-mean(motor_posx_1)), (motor_posy_1-mean(motor_posy_1)));  %stage position is saved twice. wonder if the same.
    xlabel('x-axis [\mu m]');
    ylabel('y-axis [\mu m]');
    title(act_str);
    figure; 
